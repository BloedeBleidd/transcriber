...
- **Bash 4.3+** on the host (required by `transcribe.sh` helpers such as `add_common_docker_args`, `add_common_container_args`, `add_output_args`, and `add_cookies_args`, which use `local -n` namerefs)
